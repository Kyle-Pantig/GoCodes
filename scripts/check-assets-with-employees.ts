...
async function checkAssetsWithEmployees() {
  try {
    ...
    // Get all assets with active checkouts (which have employee users)
    ...
    if (anomalies.length > 0) {
      ...
      anomalies.forEach((asset, index) => {
        const checkout = asset.checkouts[0]
        console.log(`\n${index + 1}. ${asset.assetTagId} - ${asset.description?.substring(0, 60)}...`)
        console.log(`   Status: ${asset.status || 'No Status'}`)
        console.log(`   Employee: ${checkout?.employeeUser?.name || 'N/A'} (${checkout?.employeeUser?.email || 'N/A'})`)
    ...
    assetsWithEmployees.forEach(asset => {
      ...
      if (checkout?.employeeUser) {
        const key = checkout.employeeUser.id
        if (!employeeMap.has(key)) {
          employeeMap.set(key, {
            name: checkout.employeeUser.name,
            email: checkout.employeeUser.email,
            total: 0,
            inUse: 0,
            anomalies: 0
          })

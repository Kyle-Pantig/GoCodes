...
interface ReservationReportFiltersProps {
  ...
  onFiltersChange: (filters: ReservationReportFiltersProps['filters']) => void
  disabled?: boolean
  isMobilePopover?: boolean
}

export function ReservationReportFilters({ filters, onFiltersChange, disabled = false, isMobilePopover = false }: ReservationReportFiltersProps) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={localFilters.reservationType || 'all'}
            onValueChange={(value) => handleFilterChange('reservationType', value === 'all' ? undefined : value)}
          >
            <SelectTrigger id="reservation-type-filter" className="w-full truncate">
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="employee-filter" className="w-full truncate">
              <SelectValue placeholder="All Employees" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Employees</SelectItem>
              {employees?.map((emp: { id: string; name: string; email: string }) => (
                <SelectItem key={emp.id} value={emp.id} className="truncate">
                  {emp.name} ({emp.email})
                </SelectItem>
              ))}
            </SelectContent>

...
function LeaseReportsPageContent() {
  ...
  const { data, isLoading, isFetching, error, refetch } = useQuery<LeaseReportData>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/reports/lease?${queryString}`)
      if (!response.ok) {
        throw new Error('Failed to fetch lease reports')
      }
  ...
  const handleDataExport = async (format: 'csv' | 'excel') => {
    ...
    const response = await fetch(`/api/reports/lease/export?${params.toString()}`)
  ...
  const handlePDFExport = async () => {
    ...
    if (includeLeaseList) {
      try {
        ...
        const response = await fetch(`/api/reports/lease?${params.toString()}`)
        if (response.ok) {
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6">
            <p className="text-center text-muted-foreground">
              You do not have permission to view lease reports.
            </p>
          </CardContent>
        </Card>
      </div>

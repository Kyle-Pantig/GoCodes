...
function LocationReportsPageContent() {
  ...
  const generateLocationReportHTML = (data: LocationReportData, allAssets?: typeof data.assets, includeList: boolean = false) => {
    const styles = `
      body {
    ...
    return `
      ...
      <body>
        ...
        <div class="subtitle">Asset distribution by location and site utilization</div>
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
          <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; background: #f9fafb;">
            ...
            <div style="font-size: 24px; font-weight: bold; color: #111827;">${data.summary.totalAssets}</div>
          </div>
          <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; background: #f9fafb;">
            <h3 style="margin: 0 0 5px 0; font-size: 14px; color: #6b7280;">Total Locations</h3>
            <div style="font-size: 24px; font-weight: bold; color: #111827;">${data.summary.totalLocations}</div>
          </div>
          <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; background: #f9fafb;">
            <h3 style="margin: 0 0 5px 0; font-size: 14px; color: #6b7280;">Total Sites</h3>
            <div style="font-size: 24px; font-weight: bold; color: #111827;">${data.summary.totalSites}</div>
          </div>
          <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; background: #f9fafb;">
            ...
            <div style="font-size: 24px; font-weight: bold; color: #111827;">${formatCurrency((() => {
              const allAssetsForCalc = allAssets || data.assets || []
              return allAssetsForCalc.reduce((sum, a) => sum + (a.cost || 0), 0)
            })())}</div>
          </div>
        </div>
        ...
        <table>
          ...
          <tbody>
            ${data.summary.byLocation.length === 0 ? `
              <tr>
                <td colspan="5" style="text-align: center; color: #6b7280;">No location data found</td>
              </tr>
            ` : data.summary.byLocation.map(loc => `
              <tr>
                <td><strong>${loc.location || 'Unknown'}</strong></td>
                <td>${loc.assetCount}</td>
                <td>${formatCurrency(loc.totalValue)}</td>
                <td>${formatCurrency(loc.averageValue)}</td>
                <td>${loc.utilizationPercentage.toFixed(1)}%</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        ...
        <table>
          ...
          <tbody>
            ${data.summary.bySite.length === 0 ? `
              <tr>
                <td colspan="6" style="text-align: center; color: #6b7280;">No site data found</td>
              </tr>
            ` : data.summary.bySite.map(site => `
              <tr>
                <td><strong>${site.site || 'Unknown'}</strong></td>
                <td>${site.assetCount}</td>
                <td>${formatCurrency(site.totalValue)}</td>
                <td>${site.locationCount}</td>
                <td>${formatCurrency(site.averageValue)}</td>
                <td>${site.utilizationPercentage.toFixed(1)}%</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        
        ${includeList && allAssets ? `
        <h2>Asset Details (${allAssets.length} assets)</h2>
        <table>
          <thead>
            <tr>
              <th>Asset Tag</th>
          ...
          <tbody>
            ${allAssets.length === 0 ? `
              <tr>
                <td colspan="8" style="text-align: center; color: #6b7280;">No assets found</td>
              </tr>
            ` : allAssets.map(asset => `
              <tr>
                ...
                <td>${asset.lastMoveDate ? format(new Date(asset.lastMoveDate), 'MMM d, yyyy') : 'N/A'}</td>
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-1.5 text-sm sm:text-base">
            Track assets by location, site utilization, and movement history
          </p>
        </div>
        {!isMobile && (
          ...
          >
            ...
            <Card className="pb-0 relative">
              <CardHeader>
                ...
                <CardDescription>
                  Asset distribution and utilization across locations
                </CardDescription>
              </CardHeader>
              <CardContent className="relative px-0">
                {isFetching && reportData && reportData.summary.byLocation.length > 0 && (
                ...
                <ScrollArea className="w-full">
                  <Table>
                    ...
                    <TableBody>
                      {reportData.summary.byLocation.length === 0 ? (
                        <TableRow>
                        ...
                        reportData.summary.byLocation.map((location, index) => (
                          <TableRow key={index}>
                            <TableCell className="font-medium">{location.location}</TableCell>
                            <TableCell>{location.assetCount}</TableCell>
                            <TableCell>{formatCurrency(location.totalValue)}</TableCell>
                            <TableCell>{formatCurrency(location.averageValue)}</TableCell>
                            <TableCell>
            ...
            <Card className="pb-0 gap-0 relative">
              <CardHeader>
                ...
                <CardDescription>
                  Detailed asset information with location data
                </CardDescription>
              </CardHeader>
              <CardContent className="flex-1 px-0 relative">
                {isFetching && reportData && reportData.assets.length > 0 && (
                  <div className={cn("absolute left-0 right-[10px] top-[33px] bottom-0 bg-background/30 backdrop-blur-sm z-20 flex items-center justify-center rounded-b-2xl", isMobile && "right-0 rounded-b-2xl")}>
                    <Spinner variant="default" size={24} className="text-muted-foreground" />
                  </div>
                )}
                {permissionsLoading || (isLoading && !reportData) ? (
                        ...
                        <Table className="border-b">
                          <TableHeader className="sticky -top-1 z-20 bg-card [&_tr]:border-b-0 -mr-2.5">
                            <TableRow className="group hover:bg-muted/50 relative border-b-0 after:content-[''] after:absolute after:bottom-0 after:left-0 after:right-[1.5px] after:h-px after:bg-border after:z-30">
                              ...
                              <TableHead className="text-left bg-card transition-colors group-hover:bg-muted/50">Location</TableHead>
                              <TableHead className="text-left bg-card transition-colors group-hover:bg-muted/50">Site</TableHead>
                              ...
                              <TableHead className="text-left bg-card transition-colors group-hover:bg-muted/50">Last Move</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                              {reportData.assets.map((asset) => (
                                <TableRow key={asset.id} className="group relative hover:bg-muted/90 border-b transition-colors">
                                  ...
                                  <TableCell>
                                    <Badge variant="secondary">{asset.status || 'N/A'}</Badge>
                                  </TableCell>
                                  <TableCell>
                                    {asset.lastMoveDate
                                      ? format(new Date(asset.lastMoveDate), 'MMM d, yyyy')
                                      : 'N/A'}
                                  </TableCell>
                                </TableRow>
                              ))}
                ...
                {/* Pagination Bar - Fixed at Bottom (hidden on mobile) */}
                <div className={`sticky bottom-0 border-t bg-card z-10 shadow-sm mt-auto rounded-b-lg ${isMobile ? 'hidden' : ''}`}>

...
async def create_checkout(
    ...
    try:
        ...
        # Check permission
        has_permission = await check_permission(user_id, "canCheckout")
        if not has_permission:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to checkout assets"
            )
...
async def update_checkout(
    ...
    try:
        ...
        if not has_permission:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to update checkout records"
            )
